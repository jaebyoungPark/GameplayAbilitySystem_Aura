...
TArray<FVector> UAuraSummonAbility::GetSpawnLocations()
{
	...
	//AB_LOG(LogTemp, Warning, TEXT("[Forward] : %s, [Location] : %s, [DeltaSpread] : %.2f"), *Forward.ToString(), *Location.ToString(), DeltaSpread);

	const FVector LeftOfSpread = Forward.RotateAngleAxis(-SpawnSpread / 2.F, FVector::UpVector);

	//AB_LOG(LogTemp, Warning, TEXT("[LeftOfSpread] : %s"), *LeftOfSpread.ToString());
